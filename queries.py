...
def Q_1(conn, execution_time):
    ...
    query = """
    SELECT p.name, AVG(s.statsbomb_xg) as avg_xg
    FROM players p
    JOIN shots s ON p.player_id = s.player_id
    JOIN matches m ON s.match_id = m.match_id
    JOIN seasons se ON m.season_id = se.season_id
    JOIN competitions c ON se.competition_id = c.competition_id
    WHERE c.competition_name = 'La Liga' AND se.season_name = '2020/2021'
    GROUP BY p.name
...
def Q_2(conn, execution_time):
    ...
    query = """SELECT p.name, COUNT(s.shot_id) as num_shots
        FROM players p
        JOIN shots s ON p.player_id = s.player_id
        JOIN matches m ON s.match_id = m.match_id
        JOIN seasons se ON m.season_id = se.season_id
...
def Q_3(conn, execution_time):
    ...
    query = """
    SELECT p.name, COUNT(s.shot_id) AS first_time_shots_count
    FROM players p
    JOIN shots s ON p.player_id = s.player_id
    JOIN matches m ON s.match_id = m.match_id
    JOIN seasons se ON m.season_id = se.season_id
    ...
    GROUP BY p.name
    HAVING COUNT(s.shot_id) > 0
    ORDER BY first_time_shots_count DESC; """
...
def Q_4(conn, execution_time):
    ...
    query = """
     SELECT t.team_name, COUNT(p.pass_id) AS total_passes
    FROM teams t
    JOIN events e ON t.team_id = e.team_id
    JOIN passes p ON e.event_id = p.event_id
...
def Q_5(conn, execution_time):
    ...
    query = """
    SELECT p.name, COUNT(pa.recipient_id) AS recipient_count
    FROM players p
    JOIN passes pa ON p.player_id = pa.recipient_id
    ...
    GROUP BY p.name
    HAVING COUNT(pa.recipient_id) > 0
...
def Q_6(conn, execution_time):
    ...
    query = """
    SELECT t.team_name, COUNT(s.shot_id) AS shots_count
    FROM teams t
    JOIN events e ON t.team_id = e.team_id
